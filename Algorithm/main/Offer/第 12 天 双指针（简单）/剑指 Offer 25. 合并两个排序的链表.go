...
func mergeTwoLists1(l1 *LinkList.ListNode, l2 *LinkList.ListNode) *LinkList.ListNode {
	list := &LinkList.ListNode{}
	l := list
	p, q := l1, l2
	for q != nil && p != nil {
		if p.Val < q.Val {
			l.Next = &LinkList.ListNode{Val: p.Val}
			p = p.Next
		} else {
			l.Next = &LinkList.ListNode{Val: q.Val}
			q = q.Next
		}
		l = l.Next
	}
