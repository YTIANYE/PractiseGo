...
*/
func pathSum1(root *TreeNode.TreeNode, target int) [][]int {
	pathsum := [][]int{}
	if root == nil {
		return nil
	}
...
// DFS,依次从树中的每个结点出发
func pathFind(root *TreeNode.TreeNode, target int, pathsum *[][]int) {
	if root == nil {
		return
	}
	path := []int{}
...
// 从当前节点出发的路径
func pathEach(root *TreeNode.TreeNode, target int, path []int, pathsum *[][]int) {
	if root == nil {
		return
	}
...
func pathSum2(root *TreeNode.TreeNode, target int) [][]int {
	path := []int{}
	ans := [][]int{}
	var dfs func(*TreeNode.TreeNode, int) // 递归调用自己，就需要声明，不能声明定义在一起
	dfs = func(root *TreeNode.TreeNode, left int) {
		if root == nil {
			return
		}
		left -= root.Val
...
*/
func pathSum3(root *TreeNode.TreeNode, target int) [][]int {

	path := []int{}
	pathsum := [][]int{}
	var dfs func(*TreeNode.TreeNode, int) // 注意这种写法
	dfs = func(root *TreeNode.TreeNode, left int) {
		if root == nil {
			return
		}
...
*/
func pathSum(root *TreeNode.TreeNode, target int) [][]int {
	if root == nil {
		return nil
	}
	hashmap := make(map[*TreeNode.TreeNode]*TreeNode.TreeNode) // 保存儿子节点的父亲节点    index:son  value:father
	hashmap[root] = nil

	// 找路径
	findPath := func(node *TreeNode.TreeNode) []int {
		path := []int{}
		for node != nil {
