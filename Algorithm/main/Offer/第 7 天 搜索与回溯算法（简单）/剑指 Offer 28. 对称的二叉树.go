...
func isSymmetric1(root *TreeNode.TreeNode) bool {
	if root == nil {
		return true
	}

	queue := []*TreeNode.TreeNode{root.Left, root.Right}
	for len(queue) != 0 {
		var nums []int
		l := len(queue)
		for i := 0; i < l; i++ {
...
func order(root *TreeNode.TreeNode, nums *[]int, tag bool) {
	if root == nil {
		*nums = append(*nums, -1)
		return
	}
...
func isSymmetric2(root *TreeNode.TreeNode) bool {
...
func fun(L *TreeNode.TreeNode, R *TreeNode.TreeNode) bool {
	if L == nil && R == nil {
		return true
